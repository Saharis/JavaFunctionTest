...
public class DealWithAndroidDel {
...
//		}
	 ...
	 * 
	 * @return 
	 *         ArrayList<String[]>，同名类列表，元素为String数组，该数组只有三个元素，第一个元素为包含后缀名的java类名
	 *         ，第二个和第三个元素为他们的不同路径
	 */
	private static ArrayList<String[]> processDupNameFileList() {
		ArrayList<FileModel> nameList = new ArrayList<FileModel>();
		for (int i = 0; i < Config.PATH_LIST.length; i++) {
			getFileList(nameList, new File(Config.FOLDER_PATH_ORGIN + Config.PATH_LIST[i]));
		}
